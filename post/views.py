...
class ApplyView(APIView):
    ...
    def post(self, request):
        ...
        if apply_serialzer.is_valid():
            init_status = ApplicationStatus.objects.get(status='submitted')
           
            apply_serialzer.save(status=init_status)
            return Response(status=status.HTTP_200_OK)
    ...
    def get(self, request):
        ...
        application = JobPostActivity.objects.filter(job_status__status=app_status)

        if application.exists():
            serializer = JobPostActivitySerializer(application, many=True)
            return Response(serializer.data)
